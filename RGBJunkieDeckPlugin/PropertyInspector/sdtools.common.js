...
function openWebsite(url) {
    if (websocket && (websocket.readyState === 1)) {
        var target = (url && String(url).trim()) || 'https://www.rgbjunkie.com/RGBJunkieApp/docs/app-deep-links';
        const json = {
            'event': 'openUrl',
            'payload': {
                'url': target
...
window.addEventListener('beforeunload', function (e) {
    ...
    // Persist dropdown picks that never fired change (e.g. user closes PI right after selecting).
    setSettings();
...
function initPropertyInspector() {
    var lists = document.getElementsByClassName('sdList');
    Array.prototype.forEach.call(lists, function (elem) {
